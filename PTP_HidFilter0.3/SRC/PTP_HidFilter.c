...
)
{
    ...
    RtlZeroMemory(&PtpHidDesc, sizeof(PtpHidDesc));
    WDF_MEMORY_DESCRIPTOR_INIT_BUFFER(
    ...
    KdPrint(("PtpFilterSelfManagedIoInit PtpHidDesc.DescriptorList[0].wReportLength = %x\n", PtpHidDesc.DescriptorList[0].wReportLength));

    ULONG PtpHidReportDescLength = PtpHidDesc.DescriptorList[0].wReportLength;
    if (PtpHidReportDescLength == 0) {
        KdPrint(("PtpFilterSelfManagedIoInit PtpHidReportDescLength err, %x\n", status));
        goto exit;
    }
    PUCHAR pPtpHidReportDesc = ExAllocatePoolWithTag(NonPagedPoolNx, PtpHidReportDescLength, TPFILTER_POOL_TAG);
    if (!pPtpHidReportDesc) {
